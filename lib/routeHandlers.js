    ...
    _data.read('users', email, function (err) {
      if (err) {
        ...
        if (hashedPwd) {
          ...
          callback(402, { 'Error': 'Could not hash user\'s password' })
        }
      } else {
        callback(401, { 'Error': 'A user with the provided email address exists. Please select another email address' })
      }
    });
  } else {
    callback(406, { 'Error': 'Required fields are missing or invalid' })
  }
}
    ...
    _data.read('tokens', token, function (err, data) {
      if (!err && data) {
        if (data.tokenExpires > Date.now()) {
          ...
          callback(402, { 'Error': 'Cannot update expired token' })
        }
      } else {
        callback(401, { 'Error': 'Specified token does not exist' })
      }
    })
  } else {
        ...
        if (!err && userData) {
          ...
          if (time < 12) {
            ...
            if (grilled || macaroni || cheese && !butternut && !chicken && !sweet) {
              ...
              callback(408, { 'Error': 'Shopping cart contains pizza orders that are currently unavailable or a negative order quantity. Please refer to menu to see available orders and/or make order quantity positive' })
            }
          }
        } else {
          callback(406, { 'Error': 'Could not find user with specified token' })
        }
      });
    } else {
      callback(400, { 'Error': 'Provided token is not valid or has expired' })
    }
  });
}
  ...
  if (butternut || chicken || sweet || grilled || macaroni || cheese) {
    ...
    _data.read("tokens", token, function (err, tokenData) {
      if (!err && tokenData) {
        ...
        callback(400, { 'Error': 'Provided token is either invalid or expired' })
      }
    })
            ...
            _data.read("users", email, function (err, userData) {
              if (!err && userData) {
                ...
                _data.read('shoppingCart', orderId, function (err, shopData) {
                  if (!err && data) {
                    const charge = Number(((shopData.TotalCost).slice(1)));
                ...
                cb(408, { "Error": "Could not find user with matching credentials" })
              }
            })
          } else {
            cb(409, { 'Error': 'Please provide valid token' })
          }
        })
      } else {
        cb(406, { 'Error': 'Credit card has expired. Please provide valid card for payment' })
      }
    } else {
      cb(407, { 'Error': 'Required fields not provided' })
    }
  } else {
